...
export function getAnnotations(content, searchTerm) {
	const searchRegExp = new RegExp(searchTerm, 'gi');

	const annotations = [];
	let match;
	if (searchTerm) {
		while ((match = searchRegExp.exec(content)) !== null) {
			const startOffset = match.index;
			const endOffset = searchRegExp.lastIndex;
			annotations.push({
				id: uuidv4(),
				startOffset,
				endOffset,
